...
**Pattern source**: Architecture borrows from the existing **RAD Knowledge Gateway** (`/workspace`) — specifically the ingestion orchestrator pattern, epoch-style atomic visibility, 7-stage retrieval pipeline, graph builder cycle, LLM runtime interface, and React frontend structure (TanStack Query + Zustand + Shadcn/ui). But the data domain, graph model, and connectors are entirely new.
...
- `internal/auth/` → JWT middleware pattern (adapated for OIDC)
...
**Patterns borrowed from RAD system:**
- `internal/graph/types.go` → GraphNode/GraphEdge struct patterns
- `internal/graph/builder.go` → build→validate→publish cycle
- `internal/graph/sqlite_store.go` → CRUD patterns (adapted for Neo4j)
- `internal/graph/traversal/` → graph traversal patterns
- `internal/llm/runtime.go` → LLM runtime interface (for extraction calls)
- `internal/knowledge/confidence.go` → confidence scoring patterns
...
**Key files:**
- `internal/retrieval/retriever.go` — Main orchestrator: 8-stage pipeline
- `internal/retrieval/intent_detector.go` — Enterprise intents (find_expert, find_document, find_project_info, find_technology_usage, general_question)
- `internal/retrieval/permission_filter.go` — Filter results by user's M365 permissions
- `internal/retrieval/semantic_search.go` — Embed query → search similar text chunks
- `internal/retrieval/graph_expander.go` — Expand found entities to related entities (BFS, depth 1-2)
- `internal/retrieval/reranker.go` — Rerank combined results
- `internal/retrieval/context_packer.go` — Token-aware context assembly
- `internal/retrieval/answer_generator.go` — LLM answer generation with source citations
...
**Patterns borrowed from RAD system:**
- `internal/retriever/retriever.go` → 7-stage pipeline pattern
- `internal/retriever/metadata_filter.go` → filter pattern (for permission filtering)
- `internal/retriever/intent_detector.go` → intent classification
- `internal/retriever/graph.go` → graph expansion during retrieval
- `internal/retriever/vector.go` → vector search pattern
- `internal/retriever/reranker.go` → reranking with confidence
- `internal/retriever/context_packer.go` → token-aware context assembly
- `internal/retriever/hydrator.go` → source hydration pattern
...
**Key files:**
- `internal/feedback/store.go` — SQLite-backed feedback storage
- `internal/feedback/analyzer.go` — Analytics: trending answers, low-confidence hotspots
- `internal/feedback/improver.go` — Periodic: re-scan low-confidence edges → re-extract with LLM
- `internal/feedback/exporter.go` — Export conversation pairs for fine-tuning
...
**Patterns borrowed from RAD system:**
- `internal/knowledge/confidence.go` → confidence scoring patterns
- `internal/scheduler/` → periodic job pattern for re-evaluation
- `internal/metrics/` → metrics collection pattern
...
- `src/Frontend/src/api/client.ts` → Axios client pattern
- `src/Frontend/src/hooks/useWebSocket.ts` → real-time updates
- `src/Frontend/src/pages/BrainChatPage.tsx` → Q&A chat interface
- `src/Frontend/src/pages/CodeGraph.tsx` → graph visualization
- `src/Frontend/src/store/useUIStore.ts` → Zustand state management
- `src/Frontend/src/hooks/` → TanStack Query custom hooks
- `src/Frontend/src/i18n/` → i18n (en, vi)
- `src/Frontend/src/components/ui/` → Shadcn/ui components

### Phase 6: Permission-Aware Retrieval (refinement)

**Goal:** Ensure every retrieval respects M365 permissions. This is refined from Phase 6 into a continuous concern, with full implementation in Phase 3.
