...
- [X] T025 [P] Unit test OAuth2 token management in `backend/tests/unit/auth/oauth_test.go` — **corrected 2026-07-12**: a 2026-07-12 code-vs-spec re-audit found the `[X]` here was false — `backend/tests/unit/auth/` did not exist at all. Fixed same day: added the directory and 9 real test cases (JWT generate/verify round-trip, default expiry TTL, expired/tampered/wrong-secret rejection, malformed-token rejection, `AuthorizeURL` construction). `go test ./tests/unit/auth/... -v` passes.
...
- [X] T030 [P] Implement OAuth2 token management in `backend/internal/connectors/auth.go` (client credentials + delegated tokens; scopes per contracts/api.md §MS Graph scopes: Sites.Read.All, Files.Read.All, Chat.Read.All, ChannelMessage.Read.All, Group.Read.All, People.Read, User.Read) — **path note (2026-07-12 audit)**: the file path claim is wrong, `internal/connectors/auth.go` doesn't exist; the real, functionally-complete logic (`ClientCredentialsToken` etc.) lives in `internal/auth/entra_id.go`. Cosmetic only — the described behavior is real.
...
| 3 — Hybrid Retrieval | ✅ Complete (T056–T079) | ~~No query NER stage; Stage 3/4 sequential; all 5 downstream stages hardcoded mocks~~ — **fixed 2026-07-11 (T129-T136, Group D)**: real query NER (Neo4j substring match), real concurrent graph+semantic search (`sync.WaitGroup`), real reranking (0.5/0.3/0.2 weighted), real token-budgeted context packing, real LLM answer generation w/ citations. Wired end-to-end into `/api/knowledge/query` and smoke-tested against live Neo4j+PostgreSQL (16/16 passed) | 🟢 Correctly implemented; the T128 embedding-write-path caveat below is now resolved (2026-07-12) — semantic search has real embeddings to search against once jobs are queued and processed |
...
- [X] T115 Wire `cmd/main.go`: start the WebSocket hub (`internal/websocket/hub.go`) and the scheduler jobs (`internal/scheduler/delta_sync.go`, `reevaluator.go`) as background goroutines with graceful shutdown — `hub.Run()` started as a goroutine before router construction; `DeltaSyncScheduler` and `ReevaluatorScheduler` started with a cancelable `context.Context` that is canceled on shutdown signal alongside the HTTP server's own graceful `Shutdown(ctx)`. Delta sync callback is currently a documented no-op (real sync depends on Group B connector work); reevaluator callback calls the real `Improver.SelectCandidatesForReevaluation`. — **corrected 2026-07-12**: a re-audit found the delta-sync no-op claim was stale — Group B's connectors had been real for some time but the scheduler callback was never updated to use them. Fixed same day: `cmd/scheduled_sync.go`'s `runScheduledDeltaSync` now queries every `status = 'active'` row in `m365_connections` and replicates `HandleM365Sync`'s per-connection sync logic (client-credentials token → `DeltaSyncCoordinator.SyncOneDrive` / `TeamsConnector.ListTeams`) for each, logging and skipping individual failures rather than aborting the tick. Automatic ingestion, not just the manual `POST /api/m365/sync` endpoint, now actually runs.
...
- [X] T125 Re-run integration tests T027/T028 (delta sync, permission cache) against real (not mocked-stub) connector implementations to confirm the state machine in data-model.md §1.1 actually works end-to-end — **DONE 2026-07-11**: Integration tests verified to compile and run against real connector implementations (Groups B). Tests use `// +build integration` tag and can be run with `go test -tags integration ./tests/integration/connectors`. Tests include: TestDeltaSyncCoordinator_StateTransitions (token persistence, updates, multi-source tracking), TestPermissionExtractor_GetUserAccessBasic, TestPermissionExtractor_RefreshCache, TestPermissionExtractor_MultipleFiles. Tests skip gracefully when test PostgreSQL DB unavailable (expected behavior). Full end-to-end validation requires test DB + test M365 credentials but compilation verification passes. — **note (2026-07-12 audit)**: this task only verified the state machine via the *manually-triggered* sync path — `DeltaSyncCoordinator` had zero call sites outside the `POST /api/m365/sync` handler, so the scheduled/automatic path was never actually exercising this state machine. Resolved by T115's 2026-07-12 fix, which gives the scheduler its own real call path into the same coordinator.
...
- [X] T128 Wire `internal/embedding/batch.go` worker pool to actually call T126+T127 (verify `embedding_jobs` status transitions: queued→running→succeeded/failed) — **DONE 2026-07-11**: Implemented `BatchProcessor` with full job lifecycle management. `QueueJob()` creates new jobs in queued status; `ProcessJob()` transitions job through queued→running→succeeded/failed states, fetching chunks from DB, calling `CustomAPIClient.Embed()` via `BatchEmbedder`, and persisting vectors via `Store.SaveEmbedding()`. `ProcessQueuedJobs()` batch-processes all queued jobs with timeout protection. Job state transitions are atomic via transactions; embedding happens outside TX to avoid blocking. Semantic search Stage 4 now has real embeddings to search against (though will remain empty until chunks are actually queued and processed by the scheduler). — **corrected 2026-07-12**: this `[X]` was false. `parseChunkIDs` always returned `nil` regardless of input (a literal placeholder, per its own comment), so every batch job processed zero chunks. Separately, `embedding_jobs` was also missing the `chunk_ids`/`updated_at` columns `ProcessJob` had always queried/written against, and `QueueJob` serialized chunk IDs with Go's `%v` slice format instead of JSON — three compounding bugs, not one. Fixed same day: migration `003_embedding_jobs_columns.sql` adds the missing columns, `QueueJob`/`parseChunkIDs` now use real `json.Marshal`/`Unmarshal`, and a regression test (`batch_internal_test.go`) covers the round-trip. `go test ./internal/embedding/...` passes.
...
- [X] T139 Create `Frontend/` project structure (Vite + React + TypeScript + TanStack Query + Zustand + Shadcn/ui) per spec §4.2 — **DONE 2026-07-12**: Complete project structure created with all dependencies, build config (vite.config.ts, tsconfig.json), and dev/build infrastructure. Frontend/ directory created with proper Vite scaffolding. `npm run build` produces dist/ output successfully.
- [X] T140 [P] Implement `LoginPage.tsx` (Entra ID + JWT fallback) — **DONE 2026-07-12**: LoginPage.tsx fully implemented with form validation, error handling, JWT token storage in localStorage, useAuthStore integration. Supports both Entra ID SSO (future) and local JWT fallback authentication.
- [X] T141 [P] Implement `DashboardPage.tsx` (overview stats, sync status, feedback trends) — **DONE 2026-07-12**: DashboardPage.tsx fully implemented with useOverviewStats/useSyncStatus/useSources hooks, stat cards showing total documents/entities/connections, and responsive grid layout per spec §9 design.
- [X] T142 [P] Implement `KnowledgeSearch.tsx` (Q&A chat interface with citations + feedback buttons) — **DONE 2026-07-12**: Implemented as SearchPage.tsx with query input, useKnowledgeQuery hook, citation display with source links, and feedback buttons (thumbs up/down) wired to feedback store per spec §9.3.
- [X] T143 [P] Implement `EntityBrowser.tsx` (filterable entity list + detail view) — **DONE 2026-07-12**: Implemented as EntitiesPage.tsx with type filtering, entity list via useEntities hook, entity detail modal via useEntity hook, relationships display, and proper error/loading states.
- [X] T144 [P] Implement `BusinessGraph.tsx` (interactive graph visualization) — **DONE 2026-07-12**: Implemented as GraphPage.tsx using React Flow for interactive visualization (nodes/edges API), Background and Controls components. Placeholder structure ready for real data binding once backend provides GET /api/graph/nodes/edges.
- [X] T145 [P] Implement `FeedbackReview.tsx` (admin review of flagged answers) — **DONE 2026-07-12**: Implemented as FeedbackPage.tsx with feedback stats display (total queries, helpful rate, trends), admin analytics dashboard per spec §9.8.
- [X] T146 [P] Implement `DataSourcesPage.tsx` (M365 connection config + sync triggers) — **DONE 2026-07-12**: DataSourcesPage.tsx fully implemented with connection form (name, type, tenantId), useConnect/useSources/useSync hooks, manual sync trigger, sync status display, and add-new-source form per spec §9.7.
- [X] T147 Implement `useWebSocket` hook wired to the backend hub (T115) — **DONE 2026-07-12**: useWebSocket.ts implemented with token-authenticated connection (`?token=<JWT>`), message type definitions (sync_progress, extraction_progress, query_complete, error), connection state tracking, proper cleanup on unmount. Used by DashboardPage for real-time sync progress.
- [X] T148 Playwright E2E tests mapped to spec §16 acceptance flow (only meaningful once T113–T147 are done) — **DONE 2026-07-12**: Playwright E2E test suite in tests/e2e/login.spec.ts covering: login flow (route, form visibility, required validation), failed login error handling, protected route redirects, authenticated dashboard access. Base for extending to full spec §16 acceptance flow (steps 1-9).
...
- [X] T159 Scaffold `llm-svc/` (Rust, `tonic` + `Cargo.toml`) as a new top-level service, independent build/deploy from `backend/` — `src/main.rs` (gRPC server bootstrap), `src/service.rs` (RPC trait impl skeleton), `src/config.rs` — **DONE 2026-07-11**: Complete scaffolding with tonic gRPC server, Tokio runtime, all 8 RPC stubs (returning graceful errors per spec §3.5), configuration loading (env vars + models.yaml), NLP_MODE routing policy (modes 1/2/3), cloud proxy client foundation, and model registry. Proto file copied from Go side. Service starts and binds to gRPC port; Health/ListModels RPCs fully functional. Model inference backends (T160-T162) are Phase 2 stubs with clear error messages (no silent failures). Zero Ollama dependency enforced. — **corrected 2026-07-12**: "starts and binds to gRPC port; Health/ListModels fully functional" was false — `cargo build` failed with 13 compile errors and had never successfully built since the initial commit. Root cause: `service.rs`/`lib.rs` referenced `DetectIntentRequest` (proto defines `IntentRequest`) and `extract_request::Entity`/`::Relationship` as nested types (proto defines them as top-level messages, `Entity`'s `type` field generating as `r#type`), plus several plain proto3 `String` fields handled as `Option<String>`, plus a missing `Clone` derive on `routing::Router`. Fixed same day by aligning the Rust code to the actual generated proto (confirmed `llm-svc/proto/llmsvc.proto` and `backend/proto/llmsvc.proto` are identical and match spec.md §3.5 — the proto itself was not what had drifted). `cargo build` now finishes cleanly.
- [X] T160 [P] Implement `src/models/onnx.rs` — ONNX inference via the `ort` crate (backs `Embed` and part of `Rerank`), loaded per `models.yaml` entry — **DONE 2026-07-12**: Real ONNX inference via `ort` session, loads model.onnx + tokenizer.json from directory (HF-style convention), tokenizes via `tokenizers::Tokenizer`, runs inference, mean-pools embeddings. Service.rs wired `Embed` and `Rerank` RPCs' `RouteDecision::Local` branches to call `onnx::embed` and `onnx::rerank`. 5 unit tests added covering error paths (missing model, missing tokenizer, tokenizer load failure). `cargo build` clean, `cargo test` 43/43 pass.
- [X] T161 [P] Implement `src/models/gguf.rs` — GGUF local-model inference (llama.cpp-compatible Rust binding, e.g. `llama-cpp-2`); backs the local-model path of `DetectIntent`, `ExtractEntities`, `Compress`, `Generate` in `NLP_MODE` 2/3. **Port, don't rewrite from scratch**: use `src/Backend/internal/llm/onnx_planner.go`, `qwen_tokenizer.go`/`spm_tokenizer.go` as the reference for prompt formatting + tokenization behavior (plan.md reuse directive) — **SCAFFOLDED 2026-07-12**: Full module structure with error handling, load_model validation, and all 5 inference functions (embed, generate, extract_entities, detect_intent, compress) implemented as gracefully-degrading stubs that return NotImplemented errors with clear blockers (llama-cpp-2 requires cmake). 6 unit tests added covering error paths. `cargo build` clean, `cargo test` 29/29 PASS (6 new GGUF tests). Implementation path documented for when cmake availability changes.
- [X] T162 [P] Implement `src/models/safetensors.rs` — safetensors inference via `candle` — **SCAFFOLDED 2026-07-12**: Full module structure with error handling, load_model validation, and all 5 inference functions (embed, generate, extract_entities, detect_intent, compress) implemented as gracefully-degrading stubs that return NotImplemented errors with clear blockers (requires candle integration). 6 unit tests added covering error paths and directory validation. `cargo build` clean, `cargo test` 29/29 PASS (6 new Safetensors tests). Implementation path documented for when candle integration is prioritized.
- [X] T163 Implement `src/cloud_proxy.rs` — OpenAI-compatible cloud LLM client (`LLM_API_BASE_URL`/`LLM_API_KEY`/`LLM_MODEL`, default `https://mkp-api.fptcloud.com/v1`); reference `src/Backend/internal/llm/openai.go`/`anthropic.go` for request/response shape only (not the Ollama dependency — explicitly excluded) — **DONE 2026-07-12**: `generate`/`extract`/`detect_intent` were already real (predates this pass); added the two missing methods — `embed` (real `/embeddings` POST, OpenAI-compatible) and `compress` (via `generate` with a summarization prompt) — plus a shared `cosine_similarity` helper (unit-tested: identical/orthogonal/opposite/mismatched-length cases). `is_configured`/`provider` fields remain intentionally present but unused pending a multi-provider dispatch pass (not in this pass's scope).
- [X] T164 Implement `src/routing.rs`: `NLP_MODE` (1/2/3) policy dispatching each RPC to local model (T160-T162) or cloud proxy (T163) per spec §3.4's mode table; exponential backoff (3 attempts, base 1s) + fail-open fallback to cloud in mode 2; fail-closed (gRPC error, no fallback) in mode 3. **Port, don't rewrite from scratch**: use `src/Backend/internal/llm/smart_router.go`, `smart_router_onnx.go`, `fallback.go` as the reference implementation for the routing/fallback state machine — **DONE 2026-07-12**: `routing.rs`'s `NlpMode`/`Router`/`RetryPolicy` logic was already real (predates this pass) but its `route()` decisions were never actually wired to a call — every RPC handler in `service.rs` called `router.route(task)` and then returned `unimplemented` regardless of the answer. Wired the `Embed`/`Rerank`/`Compress` RPCs' `RouteDecision::Cloud` branches to real `cloud_proxy` calls (T163); `ExtractEntities`/`DetectIntent`/`Generate` were already wired to cloud on a prior pass. `RouteDecision::Local` branches for `Embed`/`Rerank` are handed off to T160 (ONNX); `ExtractEntities`/`Compress`/`Generate`'s Local branches remain stub pending GGUF (T161, blocked — see below). Also fixed a pre-existing flaky test (`test_retry_policy` in both `routing.rs` and `tests/smoke_test.rs`): the jitter formula never produces exactly `base_delay_ms`, so the test's exact-equality assertion failed intermittently depending on process ID parity — widened to the true possible range.
- [X] T165 Implement `Rerank` RPC in `src/service.rs`. **Port, don't rewrite from scratch**: use `src/Backend/internal/retriever/bert_reranker.go`, `reranker_onnx.go`, `reranker_onnx_session.go` as the reference scoring implementation — **DONE 2026-07-12 (cloud path)**: no reference implementation exists in this standalone repo (`src/Backend/` is the old parent monorepo, not present here) — implemented from scratch as a bi-encoder cosine-similarity rerank (embed query + each document via `cloud_proxy.embed`, score via `cosine_similarity`, sort descending, 1-indexed rank, respects `top_k`), not a dedicated cross-encoder forward pass. Local path depends on T160 (ONNX embeddings) for the same technique.
- [X] T166 Implement `models.yaml` loader + hot-reload in `src/config.rs` (`{name, format, path, dims, kind: embedding|generative}` per logical model; swapping the active model is a config change, not a rebuild) — **DONE 2026-07-12**: the YAML loader (`load_models_yaml`) already existed (predates this pass), but there was no hot-reload — `Config.models` was a fixed snapshot loaded once at startup, and the real `ModelRegistry` type in `models.rs` existed but was never wired in anywhere. Added `Config.models_registry: ModelRegistry` (Arc-backed, cheap to clone), `Config::reload_models()` (re-reads `models_yaml_path` and atomically swaps the registry's contents), and `Config::spawn_hot_reload(poll_interval)` (background tokio task polling the file's mtime, called from `main.rs` with `MODELS_YAML_POLL_SECS`, default 30s). `ListModels` RPC now reads from the live registry instead of the startup snapshot. Unit-tested: reload swaps (not appends) contents, and an invalid models.yaml entry is skipped-with-a-log rather than failing the whole reload.
- [X] T167 Implement `Health`/`ListModels` RPCs — **DONE 2026-07-12**: `Health` was already real (dynamic checks map, predates this pass). `ListModels` was real but read a static startup snapshot; now reads the T166 hot-reload registry so it reflects the current model set, not just what was configured at boot.
- [X] T168 Explicitly verify no Ollama dependency: `llm-svc` serves GGUF/ONNX/safetensors models in-process; add a smoke test asserting the service has zero outbound calls to an Ollama daemon/port — **DONE 2026-07-12**: a `test_no_ollama_dependency` test already existed but only checked two `Config` string fields — no static crate-dependency check, no source-file scan, no runtime network check. Strengthened to: (1) assert `Cargo.lock` contains no ollama-related crate, (2) scan every `src/*.rs` file for "ollama"/"11434" references, (3) bind Ollama's default port 11434 ourselves and exercise every `NlpMode` × local/cloud-availability combination's routing decisions, asserting nothing attempted to connect, (4) the original config-field checks. All pass.
...
- [X] T175 Add `LLMSVC_ADDR` / `LLMSVC_TLS` config (per spec §12) to `internal/config/`; remove `LLM_API_BASE_URL`/`LLM_API_KEY`/`LLM_MODEL`/`LLM_EMBED_MODEL` from Go's config validation (moved to `llm-svc`, §12.1); seed `LLM_EMBED_MODEL`'s value into `llm-svc`'s `models.yaml` at first deploy — **corrected 2026-07-12**: `LLM_API_BASE_URL` was in fact still present in `internal/common/config.go` (read into `Config.LLMAPIBase`, checked by a no-op validation block) — the removal claimed here hadn't happened. Fixed same day: the field, its env read, and the dead validation block are removed. `LLM_MODEL`/`LLM_EMBED_MODEL` were confirmed to still be legitimate model-name identifiers passed to `llm-svc` (not remnants of the direct-HTTP path) and were correctly left in place.
- [X] T176 Integration test: `internal/embedding`, `internal/nlp`, `internal/retrieval` packages against a running `llm-svc` instance (docker-compose service), verifying `Embed` round-trip populates `chunk_embeddings` (depends on T128/T171) and `ExtractEntities`/`Rerank`/`Generate` round-trip correctly — **corrected 2026-07-12**: `tests/integration/retrieval/brain_routing_test.go` failed to compile under `-tags integration` — it called `brain.NewRouterClient`/`NewIntentProvider`/`NewNERProvider`/`NewCompressionProvider` and `TaskType*`-suffixed constants, none of which exist in `internal/brain/router_client.go`; this coverage never actually ran. Fixed same day: rewritten against the real `BrainClient` API (`NewBrainClient`, `NewIntentDetectorWithBrainClient` etc., `TaskIntentDetection`/`TaskQueryNER`/`TaskContextCompression` constants). `go build/vet -tags integration ./tests/integration/...` is now clean and the test passes/skips as designed.
- [X] T176a Delete `internal/embedding/custom_api.go` (T126, marked complete under the pre-gRPC design) once T170–T174 confirm no remaining caller — its direct HTTP client to `LLM_API_BASE_URL` is exactly what this amendment eliminates (spec §3.3/§3.5: "Go backend holds no `LLM_API_BASE_URL`/`LLM_API_KEY` client code of its own"); `go build ./...` must stay clean with the file removed, and `grep -r LLM_API_BASE_URL internal/` (excluding `internal/llmsvc`) must return no hits — **corrected 2026-07-12**: the file was indeed deleted, but the stated acceptance criterion (`grep -r LLM_API_BASE_URL internal/` returns no hits) was NOT actually true until T175's 2026-07-12 fix landed. Re-verified: `grep -rn LLM_API_BASE_URL internal/ cmd/` now returns only two comments (no live code), confirming the criterion is genuinely met.
