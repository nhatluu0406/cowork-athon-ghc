...
- [x] **D2 sign-off** (external M365KG stack, thin-client model vs. bundling the stack into the Windows installer) — Decision: **External stack** (thin-client REST model) — Signed off by: DungPham — Date: 2026-07-13 — **⚠️ REVERSED 2026-07-13**: same-day, DungPham reversed this to **bundle + self-provision** the stack (Postgres/Neo4j/backend/`llm-svc` as Cowork-managed child processes, portable no-admin binaries, no end-user install). See `spec.md` D2' and `docs/architecture/decisions/0010-m365kg-stack-bundling.md` (source of truth for the new decision + license review + lifecycle design). D1/tool-schema/T0.4 sign-offs above are unaffected by this reversal.
