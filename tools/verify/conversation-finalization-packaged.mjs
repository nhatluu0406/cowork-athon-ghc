/**
 * Conversation finalization packaged verification (CGHC session finalization fix).
 *
 * Journeys:
 *  A — simple question → final assistant text + completed
 *  B — file create + approve → final text + spinner ends + file exists
 *  C — safe write + deny → terminal + no file
 *  D — relaunch → persisted final message, not running
 *
 * Live inference budget: ≤3 successful completions.
 */
...
const FIXTURE_CONTENT = "CGHC_FINAL_RESPONSE_OK";
const DENY_FILE = "cghc-deny-final.txt";
...
async function onboard(fixture, profileDir) {
  ...
  await waitForText(".workspace-context", /cghc-final-ws-/i);
...
async function assertAssistantHasText() {
  const text = await lastAssistantText();
  if (text.length === 0) throw new Error("assistant bubble empty");
  return text;
}
...
async function main() {
  ...
  const fixture = mkdtempSync(join(tmpdir(), "cghc-final-ws-"));
  mkdirSync(fixture, { recursive: true });
  const profileDir = mkdtempSync(join(tmpdir(), "cghc-final-profile-"));

  let proc = await onboard(fixture, profileDir);

  console.log("finalization: journey B — file action (PO repro, first message)");
  const filePrompt = `Tạo file ${FIXTURE_FILE} với nội dung ${FIXTURE_CONTENT}.`;
  await sendPrompt(filePrompt);
  ...
  const assistantB = await assertAssistantHasText();
  console.log(`finalization: journey B assistant (${assistantB.length} chars)`);
  liveSuccessCount += 1;

  const assistantBeforeRelaunch = assistantB;

  console.log("finalization: journey A — simple response (new conversation)");
  await cdpEvaluate(`document.querySelector('.sidebar__new-btn')?.click()`);
  await sleep(1000);
  await sendPrompt("Trả lời đúng một từ: BONG.");
  await waitForCompleted();
  const assistantA = await assertAssistantHasText();
  console.log(`finalization: journey A assistant (${assistantA.length} chars)`);
  liveSuccessCount += 1;

  console.log("finalization: journey C — deny (new conversation)");
  await cdpEvaluate(`document.querySelector('.sidebar__new-btn')?.click()`);
  await sleep(1000);
  rmSync(join(fixture, DENY_FILE), { force: true });
  await sendPrompt(`Tạo file ${DENY_FILE} với nội dung DENIED trong workspace.`);
  await waitTerminalAfterPermission("deny");
  if (existsSync(join(fixture, DENY_FILE))) throw new Error("deny file should not exist");
  await assertNotProcessing();
  liveSuccessCount += 1;

  console.log("finalization: journey D — relaunch persistence");
  await stopAll(proc);
  proc = launch(
    {
      COWORK_GHC_STARTUP_TRACE: TRACE,
  ...
  await waitForSelector(".app-shell");
  await waitForText(".workspace-context", /cghc-final-ws-/i);
  await waitForSelector(".history-item");
  const selected = await cdpEvaluate(`(() => {
    const items = Array.from(document.querySelectorAll('.history-item'));
    const match = items.find((item) => /test\\.txt|CGHC_FINAL/i.test(item.textContent || ''));
    (match ?? items[0])?.click();
    return true;
  })()`);
  if (selected !== true) throw new Error("could not select conversation after relaunch");
  await sleep(1500);
  await assertNotProcessing();
  const persisted = await lastAssistantText();
  if (persisted.trim().length === 0) {
    ...
    if (all.trim().length === 0) throw new Error("no persisted assistant messages after relaunch");
    if (!all.includes(assistantBeforeRelaunch.slice(0, Math.min(20, assistantBeforeRelaunch.length)))) {
      throw new Error("relaunch did not preserve prior assistant text");
    }
  } else if (!persisted.includes(assistantBeforeRelaunch.slice(0, Math.min(15, assistantBeforeRelaunch.length)))) {
    throw new Error("relaunch did not preserve prior assistant text");
  }
  ...
  console.log(`conversation-finalization-packaged: PASS live_success=${liveSuccessCount}`);
}
...
main().catch(async (e) => {
  try {
    ...
    const msgs = String(
      await cdpEvaluate(`Array.from(document.querySelectorAll('.msg .msg__text')).map((n) => n.textContent || '').join(' | ')`),
    );
    console.error(`debug status=${status} assistantLen=${assistant.length} msgs=${msgs.slice(0, 200)}`);
  } catch {
    // ignore debug failures
  }
  console.error("conversation-finalization-packaged: FAIL", e instanceof Error ? e.message : e);
  try {
