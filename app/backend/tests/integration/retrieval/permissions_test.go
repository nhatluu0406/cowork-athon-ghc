...
func TestPermissionCache_RefreshTimestamps(t *testing.T) {
	...
	err = db.QueryRowContext(ctx,
		`SELECT last_sync_at FROM permission_cache WHERE user_id = $1`).Scan(&oldLastSync)
	if err != nil {
	...
	_, err = db.ExecContext(ctx,
		`UPDATE permission_cache SET last_sync_at = $1 WHERE user_id = $2`,
		refreshTime, "user-1")
	if err != nil {
	...
	err = db.QueryRowContext(ctx,
		`SELECT last_sync_at FROM permission_cache WHERE user_id = $1`).Scan(&newLastSync)
	if err != nil {
...
func TestPermissionFilter_PopulatedCache(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		...
		err := db.QueryRowContext(ctx,
			...
			fmt.Sprintf("item-%d", i), fmt.Sprintf("doc-%d.docx", i)).Scan(&fid)
		if err != nil {
	...
	allowed2, err := pf.Filter(ctx, "user-2")
	if err != nil {
	...
	allowed3, err := pf.Filter(ctx, "user-3")
	if err != nil {
...
func TestPermissionFilter_WithStalenessCheck(t *testing.T) {
	...
	allowed15min, err := pf.FilterWithStalenessCheck(ctx, "user-1", 15*time.Minute)
	if err != nil {
