...
// MaxFileSize is the hard cap on a single file's bytes read into memory during extraction.
// A file above this is skipped (flagged binary), never read — this bounds memory so a huge file
// (VM image, log, DB dump) in an imported folder cannot OOM the backend. `os.ReadFile` here loads
// the whole file, so the cap must be enforced BEFORE the read.
const MaxFileSize int64 = 25 * 1024 * 1024 // 25 MiB

// ErrFileTooLarge is returned when a file exceeds MaxFileSize.
var ErrFileTooLarge = errors.New("file exceeds MaxFileSize")
...
// Extract reads a file and extracts text content. Files above MaxFileSize are refused BEFORE the
// read (memory bound); the caller treats the returned error as "skip this file".
func (e *Extractor) Extract(ctx context.Context, absPath string) (*ExtractResult, error) {
	if info, statErr := os.Stat(absPath); statErr == nil && info.Size() > MaxFileSize {
		return &ExtractResult{Error: ErrFileTooLarge, IsBinary: true}, ErrFileTooLarge
	}
