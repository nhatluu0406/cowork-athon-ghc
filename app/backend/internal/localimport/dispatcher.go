...
// jobTimeout bounds a single import job so a hung scan cannot occupy a worker indefinitely.
...
func (d *Dispatcher) processJob(ctx context.Context, job *ImportJob, workerID int) {
	...
	// M4: bound every job — a scan that hangs (e.g. a stuck network drive) must not block a
	// worker forever. On timeout the context cancels; the processor honors ctx.Done().
