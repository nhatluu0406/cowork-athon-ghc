...
func DetectEncoding(sample []byte) (string, float64, error) {
	...
	// A definitive UTF-8 validity check beats chardet's heuristic, which reports LOW confidence
	// on short ASCII/UTF-8 text (small .md/.txt files) and would then wrongly flag them binary
	// — the whole file would be skipped from indexing. ASCII is valid UTF-8 so this covers the
	// common case; a NUL byte still forces the binary path (genuine binaries are not indexed).
	if len(sample) > 0 && utf8.Valid(sample) && !bytes.ContainsRune(sample, 0) {
		return "UTF-8", 1.0, nil
	}
