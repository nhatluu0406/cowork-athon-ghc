...
// requireUserID verifies the caller's "Authorization: Bearer <JWT>" the same way the M365/graph
// handlers do (the only trustworthy identity source). On failure it writes 401 and returns
// ok=false; the caller must stop. A nil JWTAuth is treated as "no auth configured" → 401 (fail
// closed): the local-import surface can read arbitrary files, so it is never left unauthenticated.
func requireUserID(w http.ResponseWriter, r *http.Request, jwtAuth *auth.JWTAuth) (string, bool) {
	if jwtAuth == nil {
	...
	claims, err := jwtAuth.VerifyToken(strings.TrimSpace(strings.TrimPrefix(header, prefix)))
	if err != nil {
		http.Error(w, "unauthorized", http.StatusUnauthorized)
		return "", false
	}
...
// authed wraps a handler with the JWT gate so every local-import route requires a verified caller.
func authed(deps *LocalImportDeps, next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if _, ok := requireUserID(w, r, deps.JWTAuth); !ok {
			return
		}
		next(w, r)
	}
...
// NewLocalImportHandler creates HTTP handlers for local import endpoints. Every route is behind
// the JWT auth gate (authed).
func NewLocalImportHandler(deps *LocalImportDeps) http.Handler {
	mux := http.NewServeMux()

	mux.HandleFunc("POST /api/local/sources", authed(deps, handleCreateSource(deps)))
	mux.HandleFunc("GET /api/local/sources", authed(deps, handleListSources(deps)))
	mux.HandleFunc("GET /api/local/sources/{id}", authed(deps, handleGetSource(deps)))
	mux.HandleFunc("PATCH /api/local/sources/{id}", authed(deps, handlePatchSource(deps)))
	mux.HandleFunc("DELETE /api/local/sources/{id}", authed(deps, handleDeleteSource(deps)))
	mux.HandleFunc("POST /api/local/sources/{id}/sync", authed(deps, handleSyncSource(deps)))
	mux.HandleFunc("GET /api/local/jobs", authed(deps, handleListJobs(deps)))
	mux.HandleFunc("GET /api/local/jobs/{id}", authed(deps, handleGetJob(deps)))
...
func handleCreateSource(deps *LocalImportDeps) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		// Auth is enforced by the `authed` wrapper in NewLocalImportHandler.
...
func handleSyncSource(deps *LocalImportDeps) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Create a new job. The partial unique index (source_id WHERE status IN
		// ('queued','running')) closes the TOCTOU window the HasRunning pre-check leaves open:
		// a concurrent sync that raced past the check fails here and is reported as 409.
		job, err := deps.JobStore.Create(r.Context(), id)
		if err != nil {
			if errors.Is(err, ErrJobAlreadyActive) {
				w.Header().Set("Content-Type", "application/json")
				w.WriteHeader(http.StatusConflict)
				json.NewEncoder(w).Encode(map[string]interface{}{"error": "job_running"})
				return
			}
