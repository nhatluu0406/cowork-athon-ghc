...
CREATE INDEX idx_local_files_source_id    ON local_files(source_id);
...
CREATE INDEX idx_chunks_local_file_id ON chunks(local_file_id) WHERE local_file_id IS NOT NULL;
CREATE INDEX idx_chunks_source_type   ON chunks(CASE WHEN local_file_id IS NOT NULL THEN 'local' ELSE 'm365' END);
