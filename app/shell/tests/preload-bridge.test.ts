...
test("the exposed bridge has EXACTLY the intended keys — no more", () => {
  ...
  assert.deepEqual(Object.keys(bridge).sort(), ["connectLive", "getBootstrap", "isSecureAutoUnlockAvailable", "pickWorkspaceFile", "pickWorkspaceFolder", "previewClose", "previewHide", "previewLoad", "previewReload", "previewSetBounds", "saveTextFile", "setDevToolsEnabled", "setStartupAuthMode", "setWindowTheme"]);
});
...
test("exposeShellBridge publishes the bridge under the shared contract key", () => {
  ...
  assert.deepEqual(
    ...
    ["connectLive", "getBootstrap", "isSecureAutoUnlockAvailable", "pickWorkspaceFile", "pickWorkspaceFolder", "previewClose", "previewHide", "previewLoad", "previewReload", "previewSetBounds", "saveTextFile", "setDevToolsEnabled", "setStartupAuthMode", "setWindowTheme"],
  );
