...
test("the exposed bridge has EXACTLY the intended keys — no more", () => {
  ...
  assert.deepEqual(Object.keys(bridge).sort(), ["connectLive", "getBootstrap", "isSecureAutoUnlockAvailable", "openExternal", "pickWorkspaceFile", "pickWorkspaceFolder", "previewClose", "previewHide", "previewLoad", "previewReload", "previewSetBounds", "saveTextFile", "setDevToolsEnabled", "setStartupAuthMode", "setWindowTheme"]);
});
...
test("each bridge method invokes exactly its allow-listed channel", async () => {
  ...
  await bridge.setWindowTheme("dark");
...
test("exposeShellBridge publishes the bridge under the shared contract key", () => {
  ...
  assert.deepEqual(
    ...
    ["connectLive", "getBootstrap", "isSecureAutoUnlockAvailable", "openExternal", "pickWorkspaceFile", "pickWorkspaceFolder", "previewClose", "previewHide", "previewLoad", "previewReload", "previewSetBounds", "saveTextFile", "setDevToolsEnabled", "setStartupAuthMode", "setWindowTheme"],
  );
