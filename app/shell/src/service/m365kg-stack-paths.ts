...
export interface M365KGStackPaths {
  ...
  /**
   * Packaged-only: `<resourcesPath>/m365kg-vendor/` containing the bundled ZIPs
   * (`postgresql.zip`, `neo4j.zip`, `jre.zip`). `null` in dev mode (no bundled ZIPs).
   */
  readonly vendorRoot: string | null;
  /**
   * Packaged-only: absolute paths to the bundled pre-compiled binaries in `resourcesPath`.
   * `null` in dev mode.
   */
  readonly binaryRoots: { readonly llmSvc: string; readonly backend: string } | null;
}
...
export function resolveM365KGStackPaths(input: M365KGStackPathsInput): M365KGStackPaths {
  ...
  // Packaged-only: vendor ZIPs and pre-compiled binaries live in `resourcesPath`.
  const vendorRoot = input.isPackaged ? join(input.resourcesPath, "m365kg-vendor") : null;
  const binaryRoots = input.isPackaged
    ? {
        llmSvc: join(input.resourcesPath, "llm-svc"),
        backend: join(input.resourcesPath, "m365kg-backend"),
      }
    : null;
