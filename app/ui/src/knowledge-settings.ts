...
function statusLabel(status: KnowledgeStatusView["status"]): string {
  switch (status) {
    case "not_configured":
...
export function mountKnowledgeSettingsPanel(
  host: HTMLElement,
  config: { client: Pick<ServiceClient, "getKnowledgeStatus" | "configureKnowledgeSource" | "testKnowledgeConnection" | "disconnectKnowledgeSource"> },
): void {
  ...
  const saveButton = el("button", "knowledge-configure-save", "Lưu cấu hình") as HTMLButtonElement;
  saveButton.type = "button";
  const testButton = el("button", "knowledge-test-connection", "Kiểm tra kết nối") as HTMLButtonElement;
  testButton.type = "button";
  testButton.disabled = true;
  const disconnectButton = el("button", "knowledge-disconnect", "Ngắt kết nối") as HTMLButtonElement;
  ...
  // Event handlers
  let currentStatus: KnowledgeStatusView | null = null;
