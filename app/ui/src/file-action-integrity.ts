...
export function detectFileActionIntent(text: string): FileActionIntent | null {
  const value = text.trim();
  if (value.length === 0 || !FILE_REFERENCE.test(value)) return null;
  if (DELETE_VERB.test(value)) return "delete";
  if (MOVE_VERB.test(value)) return "move";
  if (EDIT_VERB.test(value)) return "edit";
  if (CREATE_VERB.test(value)) return "create";
  return null;
}

function reviewMatchesIntent(review: FileReviewArtifact, intent: FileActionIntent): boolean {
  if (review.operation !== intent) return false;
  switch (intent) {
    case "create":
...
export function hasVerifiedFileAction(
  ...
  return reviews.some(
    (review) =>
      review.runtimeTurnId === runtimeTurnId &&
      review.source === "runtime_tool" &&
      reviewMatchesIntent(review, intent),
  );
}

export const UNVERIFIED_FILE_ACTION_WARNING =
  "Cowork GHC chưa xác minh được thay đổi tệp. Không có bằng chứng thực thi và trạng thái tệp phù hợp, nên yêu cầu này được xem là chưa hoàn tất.";

/** Preserve the model text as context, but never present it as verified product truth. */
export function markFileActionUnverified(text: string): string {
  const response = text.trim();
  if (response.length === 0) return UNVERIFIED_FILE_ACTION_WARNING;
