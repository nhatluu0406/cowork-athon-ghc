...
function userFacingProviderError(error?: { kind?: string; message?: string; recovery?: string } | string): string {
...
function userFacingBaseUrlError(message: string): string {
  const lower = message.toLowerCase();
  if (lower.includes("ssrf")) return userFacingProviderError(message);
  if (lower.includes("https")) return "Base URL phải dùng HTTPS.";
  return message;
}
...
function setControlsBusy(
  busy: boolean,
  controls: {
    saveCredBtn: HTMLButtonElement;
    deleteCredBtn: HTMLButtonElement;
