...
const DEFAULT_PROMPT = "Reply with only the word PING.";
...
async function awaitLiveClient(deps, sleep) {
    deps.reconnect();
...
export function mountSessionPanel(container, deps) {
                ...
                stream = startEvStream({
                    ...
                    onView: (view) => {
                        ...
                        if (view.text.length > 0)
                            streamOut.textContent = view.text;
    ...
    sendBtn.addEventListener("click", () => {
        void (async () => {
            ...
            try {
                ...
                if (text.length === 0)
                    throw new Error("Nhập nội dung yêu cầu.");
                const result = await client.sendSessionMessage(sessionId, text);
                if (!result.accepted) {
                    throw new Error(result.reason === "runtime_not_attached"
...
//# sourceMappingURL=session-panel.js.map