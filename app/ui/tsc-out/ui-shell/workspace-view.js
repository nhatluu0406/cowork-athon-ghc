...
export function createWorkspaceView() {
    ...
    emptyState.append(el("h2", "workspace-empty__title", "Chọn một tệp để xem trước"), el("p", "workspace-empty__copy", "Duyệt workspace ở sidebar hoặc mở tệp từ cuộc trò chuyện."));
...
export async function openWorkspaceFileInView(view, client, file) {
    view.emptyState.hidden = true;
    const preview = view.previewBody.parentElement;
    preview.hidden = false;
    view.docTabs.replaceChildren();
    const tab = el("button", "workspace-docs__tab workspace-docs__tab--active", file.label);
    tab.type = "button";
...
export function clearWorkspaceView(view) {
    view.docTabs.replaceChildren();
