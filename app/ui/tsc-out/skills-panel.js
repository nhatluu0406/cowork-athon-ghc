...
export function mountSkillsPanel(root, client, onChanged) {
    ...
    const copy = el("p", "skills-panel__copy", "Skill là hướng dẫn local cho lượt gửi. Skill không cấp quyền mới và không chạy code.");
    const refreshButton = el("button", "label-btn skills-refresh", "Làm mới");
    ...
    async function render(skills) {
        ...
        for (const skill of skills) {
            ...
            if (skill.validationStatus === "invalid") {
                ...
                const toggle = el("button", "label-btn skill-toggle", skill.status === "enabled" ? "Disable" : "Enable");
                toggle.type = "button";
                ...
                const previewButton = el("button", "text-btn skill-preview-btn", "Xem nội dung");
                ...
                previewButton.addEventListener("click", () => {
                    ...
                    void client
                        ...
                        .then((preview) => {
                        let pre = card.querySelector(".skill-card__preview");
                        if (pre === null) {
                            pre = el("pre", "skill-card__preview");
                            card.append(pre);
                        }
