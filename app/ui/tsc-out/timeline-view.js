...
import { FILE_OP_LABEL, RECOVERY_LABEL, STATUS_LABEL, STEP_LABEL, TERMINAL_LABEL, } from "./timeline-labels.js";
...
function labelledList(className, ariaLabel) {
    const list = el("ul", className);
    list.setAttribute("aria-label", ariaLabel);
    return list;
}
/** Create the timeline DOM once and return an incremental updater. */
export function createTimelineView(container, onRecovery) {
    ...
    function renderTools(view) {
        ...
        for (const call of view.toolCalls) {
            ...
            if (call.summary !== undefined)
                item.append(el("span", "ev-tool-summary", call.summary));
    ...
    function update(view) {
        ...
        if (prev === null || prev.todos !== view.todos)
            renderTodos(view);
        if (prev === null || prev.steps !== view.steps)
            renderSteps(view);
        if (prev === null || prev.toolCalls !== view.toolCalls)
            renderTools(view);
        if (prev === null || prev.fileMutations !== view.fileMutations)
            renderFiles(view);
        if (prev === null || prev.text !== view.text)
            renderText(view);
        if (prev === null || prev.error !== view.error)
            renderError(view);
        prev = view;
    }
    return {
