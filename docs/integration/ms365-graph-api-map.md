...
## 8. Calendar (Outlook Calendar — implemented)

| API | Method | Tool | Trạng thái | Ghi chú |
|---|---|---|---|---|
| `/me/calendarView?startDateTime=&endDateTime=` | GET | `calendar_list_events` | 🟡 **CODE + UNIT** | Liệt kê event trong khoảng thời gian; scope `Calendars.ReadWrite`. Chưa live |
| `/me/events?$search=...` | GET | `calendar_search_events` | 🟡 **CODE + UNIT** | Tìm theo subject; query chỉ vào `$search` value, không vào path |
| `/me/events` | POST | `calendar_create_event` | 🟡 **CODE + UNIT** | **Write → PermissionGate** (Deny chặn thật, mirror pattern Planner/Teams); attendees + online meeting optional |

## 9. OneDrive (my drive — implemented, chỉ đọc)

| API | Method | Tool | Trạng thái | Ghi chú |
|---|---|---|---|---|
| `/me/drive/root/search(q='...')` | GET | `onedrive_search_files` | 🟡 **CODE + UNIT** | Tìm file trong OneDrive cá nhân; `q` escape single-quote. Dùng scope `Files.ReadWrite.All` sẵn có (KHÔNG thêm scope mới). Khác `sharepoint_*` (site drive) |
| `/me/drive/root/children`, `/me/drive/items/{id}/children` | GET | `onedrive_list_folder` | 🟡 **CODE + UNIT** | Liệt kê thư mục gốc hoặc con; itemId encode. Chỉ đọc — không có write tool |

## 10. Identity (resolve user / me — implemented, chỉ đọc)

| API | Method | Tool | Trạng thái | Ghi chú |
|---|---|---|---|---|
| `/users` | GET | `resolve_user` | 🟡 **CODE + UNIT** | Resolve `{userId, displayName, email}` theo tên/email (nền cho assignee/mention). Scope `User.Read.All` |
| `/me`, `/me/mailboxSettings` | GET | `get_me` | 🟡 **CODE + UNIT** | Danh tính tài khoản đang kết nối; scope `User.Read` + `MailboxSettings.Read`. Mailbox settings best-effort (thiếu → bỏ qua, không throw) |

## 11. Power Automate (implemented — NGOÀI Graph)

| API | Method | Tool | Trạng thái | Ghi chú |
|---|---|---|---|---|
| Local store (`.runtime/ms365-power-automate.json`) | — | `power_automate_list_flows` | 🟡 **CODE + UNIT** | Liệt kê flow đã đăng ký cục bộ (chỉ `name`, KHÔNG lộ URL). Store owner-only `0o600`/`0o700`. Không gọi Graph |
| Flow HTTP-request URL (host tuỳ flow) | POST | `power_automate_trigger_flow` | 🟡 **CODE + UNIT** | **NGOÀI Graph chuẩn.** URL là credential (bearer trong query) → **`SsrfPolicy.assertAllowed` chặn trước fetch**, dùng chung policy với provider HTTP connector. **Write → PermissionGate.** KHÔNG cần Graph scope |

## 12. Scope OAuth đang khai báo (`MS365_SCOPES`)

> Nguồn sự thật là hằng `MS365_SCOPES` trong `service/src/composition/compose-service.ts`. Bảng
> này phản ánh danh sách hiện đang khai báo trong code; cột "Dùng cho" gộp cả scope theo phase.

| Scope | Dùng cho | Đã live-consent? |
|---|---|---|
| `Files.ReadWrite.All` | SharePoint file read + upload; **OneDrive read** (`/me/drive`) dùng lại scope này | ❌ chưa (token test thiếu) |
| `Sites.Read.All` | Site discovery + search + Lists read | ❌ chưa — thiếu scope này là nguyên nhân 403 `/me/followedSites` |
| `Calendars.ReadWrite` | **Calendar** list/search/create (`/me/calendarView`, `/me/events`) | ❌ chưa |
| `User.Read.All` | **`resolve_user`** (`/users`) | ❌ chưa |
| `User.Read` | **`get_me`** (`/me`) | ❌ chưa |
| `MailboxSettings.Read` | **`get_me`** — mailbox settings (`/me/mailboxSettings`, best-effort) | ❌ chưa |

> **Lưu ý drift đã biết:** một số scope theo phase (Mail.Read cho Outlook, Tasks.ReadWrite cho
> Planner, Chat/Team/Channel* cho Teams) mô tả trong các mục 4–7 CHƯA có trong `MS365_SCOPES` hiện
> tại — cần bổ sung trước lượt live tương ứng. Power Automate `trigger_flow` KHÔNG cần Graph scope
> (webhook ngoài Graph, guard bằng SSRF).

## 13. Việc cần làm để chuyển 🔴/🟡 → ✅
