...
| [0013](0013-code-surface-and-workspace-relationship.md) | **Quan hệ `Code` ↔ `Workspace` = Hybrid, backend dùng chung hoàn toàn**: Workspace = file/document-centric, Code = project/developer-centric giữ surface riêng nhưng **không** có backend/session/runtime/guard/permission-gate riêng; mọi file I/O qua loopback service token-guarded (no generic IPC / no renderer `fs`); product label đổi **"Claude Code" → "Code"** (item đầu Code Phase 1); phân biệt File preview / Runtime web preview / Desktop app launch; terminal/Git/debugger/LSP/dev-server/app-launch deferred | Accepted (post-L4, additive) |
...
>
> **ADR 0013 là additive** (post-L4): chốt quan hệ `Code` ↔ `Workspace` là Hybrid với backend dùng
> chung hoàn toàn. Nó **tái dùng** ADR 0001 (một runtime/session), 0003 (loopback), 0009 (no generic
> IPC) và **không** sửa/supersede bất kỳ ADR đóng băng 0001–0006 nào ⇒ L4 giữ `COMPLETED`.
>
