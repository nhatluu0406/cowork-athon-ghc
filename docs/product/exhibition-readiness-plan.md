...
| ER-005 | P1 | Knowledge | D3 not wired/bundled; KB/KG unusable offline (dependencies-and-services §5) | KB on embedded SQLite (FTS5) + local workspace index (LF-4) | — | med | Index a folder offline; search returns results; data under data root | packaged | M | DONE (2026-07-18, code+tests+build; packaged PO obs pending): migration id:4 + `service/src/knowledge-local` + `/v1/knowledge-local`; FTS5 keyword search. **No vector/embeddings** (deferred). |
| ER-006 | P2 | Knowledge | No graph offline | Graph on node/edge tables + visualization (LF-5) | ER-005 | med | Graph builds/renders/rebuild/clear/recovery | packaged | M | DONE (2026-07-18, code+tests+build; packaged PO obs pending): deterministic contains/links_to graph → existing SVG renderer; init/sync/rebuild/clear/cancel states. |
