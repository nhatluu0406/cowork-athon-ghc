...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| ER-001 | P0 | Packaging | Exe rename config landed but not packaged-verified (audit scoped out packaging) | Run `package:win`, confirm `dist-app\win-unpacked\coworkghc.exe`, icon + display name | — | low | Packaged exe = coworkghc.exe; titlebar/installer show "Cowork GHC" | package + launch | S | TODO |
...
| ER-005 | P1 | Knowledge | D3 not wired/bundled; KB/KG unusable offline (dependencies-and-services §5) | KB on embedded SQLite (FTS+vector) + local import (LF-4) | ER-004 | med | Import a folder offline; search returns results; data under data root | packaged | M | TODO |
| ER-006 | P2 | Knowledge | No graph offline | Graph on node/edge tables + visualization (LF-5) | ER-005 | med | Graph builds/renders/rebuild/clear/recovery | packaged | M | TODO |
...
| ER-013 | P1 | Visual audit | No automated packaged UI capture exists | Build the UI-capture tool + run it (see §4) | ER-001 | med | Screenshots for all surfaces/states; checks pass | audit run | M | TODO |
...
**Approach.** Playwright-Electron **or** CDP-in-audit-mode **or** `webContents.capturePage` — chosen
after inspecting the shell. Prefer semantic selectors over screen-coordinate clicks.
