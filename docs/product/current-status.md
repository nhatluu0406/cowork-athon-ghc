...
| Code surface (tab Code) | WORKS (shared Cowork backend) | Markdown assistant trong panel Agent (#33: bảng GFM/code/heading) + gộp dòng trống thừa (#32). **Thanh phiên dùng chung** (Phiên mới + chọn phiên cũ) ở header Code và tab Workspace (#35). Panel Agent rộng hơn `clamp(420px,30vw,560px)` + ô soạn lớn hơn (#36). *"AI miss tạo .py"* là độ tin cậy tool của OpenCode pin — #36 để OPEN cho phần đó. |
...
| **D2 Microsoft 365** | PARTIAL | Manual-token chat + history + in-tab permission cards. **ChatUI dùng MarkdownView chung** (#20). Device-code OAuth gated (chưa Azure app reg). Trạng thái kết nối **phản ánh vào tóm tắt nguồn Knowledge** (#19). **PHASE 3:** liên kết ngoài (Graph Explorer / device verify URL) mở qua **shell.openExternal allowlist** (https + host Microsoft, fail-closed); trang đã kết nối hiện **tài khoản + hạn token** (giải mã claim `name`/`preferred_username`/`exp`, không lưu token) + trạng thái "cần đăng nhập lại" khi hết hạn; **OneDrive cục bộ**: phát hiện thư mục OneDrive đã đồng bộ trên máy → "Dùng thư mục OneDrive trên máy" mở làm **workspace cục bộ** (Knowledge index local, **không** Graph/đám mây). Manual-token vẫn **chỉ phiên hiện tại** (in-memory, không ghi đĩa). First-turn readiness gate + retry đã có. **Chưa live tenant/packaged; persistent OAuth session vẫn chờ app registration.** |
