...
| **D3 Knowledge — Local KB/Graph MVP** | Code+tests+build PASS, **packaged PO obs pending** | Local-first trên SQLite hiện có (migration id:4): FTS5 keyword search + đồ thị **deterministic** (workspace→folder→file `contains` + `links_to` từ link Markdown), index bounded/cancellable/incremental, secret + dep/build dir loại trừ, scoped theo workspace. **Không** Neo4j/Postgres/Docker/embeddings/LLM. Router `/v1/knowledge-local/*` đã mount; surface Knowledge thật (trạng thái/tìm kiếm/đồ thị, mở nguồn, Hỏi Cowork). Xem `local-first-strategy.md` (Option 1). |
