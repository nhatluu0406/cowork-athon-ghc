...
| Code | Phase 1 — code/tests/build PASS, packaged PO obs pending | **Hybrid (ADR 0013)** — renderer surface dùng chung backend Cowork (cùng active workspace/`WorkspaceGuard`/`PermissionGate`/OpenCode session, không backend/session/runtime riêng). Đã triển khai **Code Phase 1 — multi-file editor**: Project Explorer dùng chung navigator; editor nhiều tab (mở/chuyển/đóng, giữ buffer); **sửa + lưu** (nút "Sửa"/Ctrl+S) qua `PUT /v1/workspace/file-content` guard-confined; dirty state + hộp thoại đóng-khi-chưa-lưu (Lưu/Không lưu/Huỷ); syntax highlight; verified-mutation refresh/xung đột/deleted dùng lại File Work Review; handoff **"Mở trong Code" ↔ "Xem trong Workspace"**; active-file làm Agent context (workspace-relative, không nhồi full-tree); label đổi **"Claude Code" → "Code"**, đã gỡ chip giả "Chạy test/Commit". **Code Slice 1 — Runtime web preview (ADR 0014, code/tests/build PASS, packaged PO obs pending):** giao diện Code thiết kế lại theo visual system của Workspace (token `--cghc-*` + dark mode), bỏ hai tab "Phiên làm việc/Cách hoạt động", bố cục Explorer \| Editor/Preview \| Agent, chế độ **Code/Preview** + Output drawer, panel Agent theo composer Cowork + thu gọn được. **Preview chạy thật**: HTML/CSS/JS **tĩnh** (máy chủ loopback bounded) và **dev server** frontend (spawn `<pm> run <script>` đã allowlist + **phê duyệt lệnh tường minh**, env curated không kèm secret, dò port, timeout, dừng graceful-then-**tree-kill**, dọn khi đổi workspace/tắt app); nhúng bằng **WebContentsView hardened** (giữ nguyên CSP renderer, chỉ loopback, chặn remote-nav/popup/download/webview). **Code Slice 2 — Desktop app launch (ADR 0015, code/tests/build PASS, packaged PO obs pending):** chạy ứng dụng **Electron** của workspace (**Build / Chạy / Dừng / Khởi động lại**) như **tiến trình/cửa sổ riêng** (không nhúng), phân biệt qua selector **Web / Ứng dụng**; dùng lại nguyên runner Slice 1 (permission bắt buộc mỗi Build/Run, env curated không secret, output redact/bounded, **tree-kill không mồ côi**, dọn khi đổi workspace/tắt app) — không tạo process manager thứ hai. Chỉ nhận **Electron** (dependency + script chạy); loại khác `unsupported` trung thực. **Chưa phải IDE**: không terminal/PTY, Git client, debugger, LSP; PDF/Office/ảnh handoff sang Workspace. Focused UI + service tests (gồm test tiến trình thật) + `build:app` PASS. **Chưa claim WORKS** cho tới packaged PO observation (xem demo-acceptance). |
...
## Dispatch live-run unblock: `start.bat` bật remote + provider FPT verified — 2026-07-18

| Item | Status |
|---|---|
| Mục tiêu (PO) | QR trên **màn D1 dispatch** *hoặc* `/remote` → điện thoại vào → **chat** → **live thẳng lên desktop**. |
| Phát hiện chính | Cả chuỗi **đã build sẵn**: màn D1 có QR (`integration-view.ts` `appendRemoteQuickAccess` → `renderRemotePairing`), `/remote` có QR (`commands/registry.ts:116` `openRemotePanel`), PWA có tab chat + dispatch (`remote-gateway/pwa.ts`), route chat POST `/api/sessions/{id}/message` + SSE `GET /api/stream`, QR ưu tiên LAN URL (`remote-router.ts:110`), `lanUrls` từ NIC khi bind `0.0.0.0` (`compose-live.ts:310`). **Chặn duy nhất**: không script nào set flag env → gateway không chạy → app hiện đúng "Remote chưa bật…" (`remote-pairing-view.ts:149` tự ghi cần `CGHC_REMOTE_ENABLED=1` + `CGHC_REMOTE_LAN=1`). |
| Root-cause "Chờ tích hợp D1" placeholder | Packaged exe user đang chạy build **2026-07-16 00:38**, TRƯỚC commit `5934660` (2026-07-16 22:44) "Dispatch surface shows the live board, not the stale awaiting-D1 placeholder". Bản packaged **cũ** → vẫn hiện placeholder thay board. `state.client` luôn có khi app chạy (settings_only lẫn live) nên board hiện bất kể unlock; QR "nothing to connect" chỉ là gateway OFF (nay start.bat bật). **Bài học: bằng chứng packaged phải từ build MỚI — exe cũ 1 ngày vẫn "PASS" nhưng thiếu UI mới.** Đã rebuild `package:win` exit 0, exe mới 2026-07-18 00:29. |
| Thay đổi | **(1)** `scripts/start.bat`: mặc định set **`CGHC_REMOTE_ENABLED=1` + `CGHC_REMOTE_LAN=1`** để double-click là đủ cho demo điện thoại. **Opt-out**: set `CGHC_REMOTE_LAN=0` trước khi chạy ⇒ loopback baseline. Logic test 4 kịch bản cmd.exe. Env chain khép kín: `start.bat set` → `node cli.mjs` → Electron spawn inherit `process.env` (`commands.mjs:99`) → `main.ts startLiveService` → `compose-live.ts:181` đọc `process.env`. **(2)** `app/shell/src/main.ts`: wrap `createLiveOptionsResolver` để truyền `remoteLog: writeLifecycleLog` vào live options — diagnostics gateway (`[cowork-remote]` LAN URL / "gateway sẵn sàng") nay vào `service-lifecycle.log` (redacted, URL-only, không secret) thay vì `process.stdout` bị packaged GUI **nuốt**. Trước fix: gateway không có log file nào để debug. tsc exit 0. |
| Provider FPT (A1/A2) | Endpoint `https://mkp-api.fptcloud.com/v1` (từ `lock.MD`) **https ⇒ qua SSRF policy sẵn**, KHÔNG cần ADR 0012 dev skip. `GET /models` OK (18 model). Tool-calling probe: **6/6 model** (`DeepSeek-V4-Flash`, `GLM-5.1`, `Llama-3.3-70B-Instruct`, `Qwen3.6-27B`, `gpt-oss-120b/20b`) trả `tool_calls` đúng args ⇒ rủi ro lớn nhất (model không function-call ⇒ không có permission prompt để Allow) **đã loại**. Khuyến nghị `DeepSeek-V4-Flash` (repo có preset DeepSeek sẵn; rẻ/nhanh, hợp fan-out). |
| Cách dùng (một lần setup) | `init.bat` → `build.bat` → cấu hình provider FPT trong Settings (base_url `.../v1`, model, key nhập qua vault — **không** hardcode) → `start.bat` → app → mở màn Dispatch (hoặc gõ `/remote`) → "Tạo mã ghép nối" → quét QR trên điện thoại cùng Wi-Fi → chat. Sau lần đầu, chỉ `start.bat`. |
| Provider setup — root cause + UX fix | Provider "FCI" của user lưu `baseUrl` **đúng** (`.../v1`) nhưng `modelId: "123"` (placeholder) ⇒ verify POST `/chat/completions {model:"123"}` → FPT **404** → fail. A2 của tôi gọi HTTP thô với model ID thật nên pass — **chưa từng test đường provider-probe của app** (khác code path). Nút "Dò model" ĐÃ có (auto-discovery qua `/models`) nhưng dính **chicken-and-egg**: lưu cần model (`presets.ts:82`), dò cần profile đã lưu + key trong vault (`provider-model-discovery.ts` "no transient key"). **Fix** (`app/ui/src/provider-profiles-panel.ts`, UI-only, backend bất biến): ở Add mode, "Dò model" bật ngay khi có Base URL + Khoá API → tự lưu nháp (create + store key, model placeholder) → dò → **tự điền model đầu tiên**. User chỉ cần URL + key, không cần biết model. typecheck 0, 2/2 panel test PASS. |
| Debug shell | `scripts/debug.ps1` — launch packaged app (remote LAN + verbose) + tail **đúng path packaged** (`%LOCALAPPDATA%\Cowork GHC\service-lifecycle.log` + `data\logs\cowork-ghc.log`), tô màu error/gateway. Phát hiện khi debug: packaged GUI **nuốt stdout**; log thật ở `AppData\Local\Cowork GHC\` **không** phải `Roaming\cowork-ghc\` (path dev/CLI — nguồn nhầm lẫn ban đầu). |
| Verified live (một phần) | Startup trace bản packaged xác nhận: service → **live** (`live_ready`), remote gateway **LÊN** `listening on 0.0.0.0:57702`, LAN URL **`http://192.168.175.159:57702`** (điện thoại cùng Wi-Fi vào được) — start.bat + main.ts remoteLog fix **verify chạy qua log thật**. **Chưa** quan sát end-to-end mirror chat desktop↔phone (cần user unlock + phone). |
| Residual (security, hoãn theo PO) | LAN mode = HTTP không TLS, device token plaintext ⇒ ai sniff Wi-Fi approve được ghi file. Comment cảnh báo đã thêm vào `start.bat`; **hard gate** (từ chối LAN ngoài dev build) + TLS/cert-pinning là slice sau (ADR 0011 Open item 2.2). `lock.MD` chứa API key plaintext ở repo root — cần `.gitignore` + rotate sau demo. |
