...
|---|---|
| Workspace explorer | Planned after the current packaged File Work Review blocker is closed. Do not start Minimal Workspace Navigator until A–L passes and Product Owner issues the brief. |
...
Foundation Phase 1 và không khóa layout UI cuối. They are planned direction, not
current product capability, and must wait until the packaged File Work Review A–L
blocker is closed.
