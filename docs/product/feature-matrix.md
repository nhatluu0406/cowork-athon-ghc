...
| Knowledge (D3) | Knowledge / Knowledge Graph slot | (UI slot) | PLACEHOLDER/DORMANT | `app/ui` knowledge-view (capability slot) | `app/backend` Go + `app/llm-svc` Rust + `service/src/knowledge` (not composed) | (designed: SQLite/Neo4j/PG) | (designed loopback) | — | No | Not wired, not bundled, never run vs real binaries |
