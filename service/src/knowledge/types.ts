...
/**
 * LLM provider configuration request (POST /api/llm/config).
 * Persisted to PostgreSQL; requires server restart to take effect.
 */
export interface LLMConfigRequest {
  readonly provider: "openai" | "anthropic" | "azure" | "custom";
  readonly baseUrl?: string; // Optional: empty for default provider URLs
  readonly apiKey: string;
  readonly model: string;
  readonly nlpMode?: 1 | 2 | 3; // 1=cloud_only, 2=cloud+local, 3=local_only
}

/**
 * LLM configuration response (POST /api/llm/config).
 */
...
/**
 * Current LLM configuration view (GET /api/llm/config/current).
